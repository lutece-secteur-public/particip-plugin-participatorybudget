					...
					<#if campaignService.isDuring("VOTE") && !voteValidated >
						<p class="sub-title">
							<#if nbVotesParis &lt; maxVotes>
								<a href="jsp/site/Portal.jsp?page=search-solr&conf=projects_mdp&query=&fq=location_text%3ATout+Paris">
					...
					<#if campaignService.isDuring("VOTE") && !voteValidated >
						<p class="sub-title">
							<#if nbVotesArrond &lt; maxVotesArrdt>
								<a href="jsp/site/Portal.jsp?page=search-solr&conf=projects_mdp&fq=location_text:${user_arrondissement!}">
