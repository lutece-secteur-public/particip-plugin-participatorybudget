	...
	<#if campaignService?? > <#-- Class not instanciated when this page is indexed by SOLR. This test avoid a lot of message into logs. -->
		<#attempt>
			...
			<#else>
				...
				<div class="container">
					<div class="map-msg">
						Seuls les projets localisés à une adresse précise sont affichés sur cette carte. Pour consulter la liste complète des projets, cliquez sur le bouton "Liste" situé en haut à droite de ce message.
					</div>
...
<link href='https://api.mapbox.com/mapbox.js/plugins/leaflet-fullscreen/v1.0.1/leaflet.fullscreen.css' rel='stylesheet' />
  ...
  var arrondissements = {
    ...
    var arr = "${(arr?js_string)!""}";
    if ((arr == "") || (arr.indexOf("whole_city") !== -1) || (arr == "all_arr")) {
...
//        "url": 'https://services1.arcgis.com/yFAX7hJID4ONeUHP/arcgis/rest/services/SECTEUR_NPQV_WGS84/FeatureServer/0',
    ...
    for (var i in arrondissements) {
        ...
        var strPopup = "<p><strong>" + strArdtTitle + " :</strong></p><p>Il y a " +(arrondissements[i]["cnt"] + arrondissements[i]["geoloc_cnt"]) + " " + strProjet + " " + strArdtDescription + ", dont " + arrondissements[i]["cnt"] + " sans adresse précise.</p><p><button type=\"button\" class=\"btn btn-default\" onclick=\"onClickArdtPopup("+i+");\">Voir ces projets</button></p>";
  ...
  var baseMaps = {
      ...
      "Fond de plan grisé": positron,
      "Vue satellite" : esri_imagery
  };

  var overlayMaps = {
      "Campaign actuelle" : layergroup_currentcampaign,
      "Quartiers populaires" : layergroup_qpvqva,
      "Précédentes campaigns" : markers_oldprojects
  };
