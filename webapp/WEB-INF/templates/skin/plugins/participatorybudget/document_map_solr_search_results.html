<section id="map-result">
	...
	<div class="container">
		<div class="map-msg">
			#i18n{participatorybudget.search.map.legendProjectsWithoutAddress}
		</div>
...
<#-- *********************************************************************************** -->
<#-- * Javascript Javascript Javascript Javascript Javascript Javascript Javascript Ja * -->
<#-- * Javascript Javascript Javascript Javascript Javascript Javascript Javascript Ja * -->
<#-- *********************************************************************************** -->
...
<script type="text/javascript">
  var arrondissements = {
	...
	var arrondissemnt= arr.split("e arrond");
        map.setView(arrondissements[parseInt(arrondissemnt)]["pos"], 14);
    }
    ...
    map.addLayer(layergroup_qpvqva); // To charge all informations 
...
//        "url": 'https://services1.arcgis.com/yFAX7hJID4ONeUHP/arcgis/rest/services/SECTEUR_NPQV_WGS84/FeatureServer/0',
    ...
    for (var i in arrondissements) {
        ...
        var strArdtTitle = (i==0) ? "whole_city" : (i+ "e arrondissement");
        var strArdtDescription = (i==0) ? "pour Paris" : "dans cet arrondissement";
        var strPopup = "<p><strong>" + strArdtTitle + " :</strong></p><p>Il y a " +(arrondissements[i]["cnt"] + arrondissements[i]["geoloc_cnt"]) + " " + strProjet + " " + strArdtDescription + ", dont " + arrondissements[i]["cnt"] + " sans adresse precise.</p><p><button type=\"button\" class=\"btn btn-default\" onclick=\"onClickArdtPopup("+i+");\">Voir ces projets</button></p>";
  ...
  var overlayMaps = {
      "Voir les Projets laur&eacute;ats" : layergroup_currentcampaign,
      "Voir les Chantiers localis&eacute;s" : markers_projectsOP,
      "Quartiers Populaires" : layergroup_qpvqva
  };
  
  // parametrage et ajout du L.control.layers a la carte
  var control =  L.control.layers(baseMaps, overlayMaps, {collapsed:false}).addTo(map);
	...
	legend.onAdd = function (map) {
		...
		div.innerHTML += '<div class="map-legend-line"><div class="marker"><img src="' +  (L.Icon.Default.imagePath + '/marker-icon-brown.png') + '"></div>Chantier localise</div>';
		return div;
	};
	legend.addTo(map);
